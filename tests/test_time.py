...
from schedule.commands import ScheduleTimeError, parse_schedule_time
...
def test_relative_time_uses_a_single_absolute_reference():
    timestamp, _ = parse_schedule_time("in 2 hours", "Asia/Jakarta", now=NOW)
...
def test_explicit_offset_resolves_each_dst_fold(value, expected):
    timestamp, _ = parse_schedule_time(value, "America/New_York", now=NOW)
    assert timestamp == int(expected.timestamp())


def test_explicit_offset_cannot_make_a_dst_gap_valid():
    with pytest.raises(ScheduleTimeError, match="does not exist"):
        parse_schedule_time(
...
def test_relative_duration_is_absolute_across_dst_transition(now):
    timestamp, _ = parse_schedule_time("in 2 hours", "America/New_York", now=now)
    assert timestamp == int(now.timestamp()) + 2 * 60 * 60


def test_absolute_local_time_is_correct():
    timestamp, _ = parse_schedule_time("2026-09-13 20:00", "Asia/Jakarta", now=NOW)
